...
6. In Render, set `MS_CLIENT_ID`, `MS_CLIENT_SECRET`, and `MS_REDIRECT_URI`
   (the same callback URL from step 2) as environment variables on the web
   service. `ENCRYPTION_KEY` is auto-generated by `render.yaml`.

Until those three variables are set, the "Connect Outlook" button will fail
